...
{	
	...
	while (TRUE)
	{
		...
		if (bInicio)	//Subtono que antecede al subtono de apertura
		{
			if (bInicio)	//Nos aseguramos de que realmente hay subtono
				for (int i=0;i<500 && bInicio;i++) delay_ms(1);								
			bFlagAp = bInicio;
		}
				
		if (bFlagAp && AperPuerta)
		{	
			bFlagAp=FALSE;
