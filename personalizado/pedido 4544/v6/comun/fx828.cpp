...
void CFX828::PutToneTable (unsigned char pos, int tono)
{
	int val = pos;
	val = val <<12;
	val |= tono;
...
void CFX828::SetDecoderBandwidth (unsigned char width)
{
...
void CFX828::Init ()
{
    ...
    Clock_PutVal(1);
	chip_select_ClrVal ();
	SendChar ((unsigned char)GENERAL_RESET);
	chip_select_SetVal ();

	//Interrupción cada 100ms.
	//Cada tono de ZVEI será de duración un tick de timer.
	SendCommand8 (GENERAL_PURPOSE_TIMER,(unsigned char)70);
...
void CFX828::ProgramFastCTCSS (float tono)
{
...
int CFX828::TxZVEI (const unsigned char tono)
{
...
void CFX828::SendChar (unsigned char Chr)
{
...
void CFX828::SendCommand16(unsigned char address, unsigned int command)
{

    Clock_PutVal(1);
	chip_select_ClrVal ();
	SendChar ((unsigned char)address);
	SendChar ((unsigned char)HIBYTE (command));
	SendChar ((unsigned char)LOBYTE (command));
	chip_select_SetVal ();
}

void CFX828::SendCommand8(unsigned char address, unsigned char command)
{

    Clock_PutVal(1);
	chip_select_ClrVal ();
	SendChar ((unsigned char)address);
	SendChar ((unsigned char)command);
	chip_select_SetVal ();
}

unsigned char CFX828::ReadCommand(unsigned char address)
{
	unsigned char ret;
...
unsigned char CFX828::ReadChar (void)
{
...
void CFX828::DataOut_PutVal(unsigned char Value)
{
...
void CFX828::Clock_PutVal(unsigned char Value)
{
  if (Value) {                         /* Is it one to be written? */
