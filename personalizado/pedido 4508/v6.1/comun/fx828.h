...
#define	DCSbyte3				0x85
#define	DCSbyte2				0x86
#define	DCSbyte1				0x87
...
class CFX828
{
	public:
		...
		void Init ();
		int  TxZVEI (const unsigned char *);
		int	 TxZVEI (const unsigned char tono);
		BOOL IsTxBufferEmpty();
		void ProgramFastCTCSS (float tono);
		void CTCSS_tx_enable (unsigned char enable);
		void PutToneTable (unsigned char pos, int tono);
		void SetDecoderBandwidth (unsigned char width);
		void EnableToneDecoder (BOOL enable, BOOL mode);
		void EnableCTCSSDecoder (BOOL enable, BOOL mode);
		void IRQService ();
		char GetTone ();
		BOOL CTCSS_Detected (void);
		void SetRepeatTone (unsigned char tono = 0xe) {_RepeatTone = tono;}
	...
	private:
		...
		void Clock_PutVal(unsigned char Value);
		void SendChar (unsigned char Chr);
		unsigned char ReadChar (void);
		void DataOut_PutVal(unsigned char Value);

		CBuffer *_bufferTX;
		CBuffer *_bufferRX;
		
		unsigned char _RepeatTone;
};
#endif
