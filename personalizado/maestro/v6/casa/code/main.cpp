...
//	FX828.Mod2Enable (FALSE);
	...
	if (APERTURA_GetVal())	//¿Ha estado la línea alta fija durante 10ms?
	{
		...
		FX828.ProgramFastCTCSS (TxCTCSSTable [((TeclaPulsada.valor-1)*5)]); //Transmitimos subtono de voz
		delay_ms (150);
		
		FX828.ProgramFastCTCSS (TxCTCSSTable [((TeclaPulsada.valor-1)*5)+1]); //Transmitimos subtono de apertura
		delay_ms (3000);
