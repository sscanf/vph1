...
{	
	...
	FX828.PutToneTable (0,RxCTCSSTable[CTCSS_APERTURA]);		//Tono de apertura de puerta
	...
	while (TRUE)
	{
		...
		if (FX828.CTCSS_Detected() && !bFlag)	//Nos llaman
		{
			for (n=0;n<50 && FX828.CTCSS_Detected();n++) delay_ms(1);
			if (n>=49)
			{
				PTT_ON;
				AUDIO_ENABLE;
		...
		if (!FX828.CTCSS_Detected() && bFlag)	//Perdemos subtono
		{
			AUDIO_DISABLE;
			if (bFlag == TRUE)
			{
				for (n=0;n<500 && !FX828.CTCSS_Detected();n++) delay_ms(1);
				if (n>=499)
				{
					desconexion();
					bFlag = FALSE;
...
void MainIRQ ()
{
	...
	if (FX828.CTCSS_Detected() || tono==0)
	{
		if (!bCTCSSDetected)
			bCTCSSDetected=TRUE;	
	}
