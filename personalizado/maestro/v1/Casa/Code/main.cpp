...
//	FX828.Mod2Enable (FALSE);
	...
	{
		EnableTX (TRUE);
		FX828.ProgramFastCTCSS (67); //Transmitimos subtono de apertura
		delay_ms (1000);
...
void comunicando (void)
{
	...
	do
	{
		if (SQL_ABIERTO)
		{
			for (n=0;n<1000 && SQL_ABIERTO;n++) delay_ms(1); // Tiene que estar abierto durante 300ms
			if (n>999)
				break;
		}
	...
	if (even_timeout)
	{
		...
		while (SQL_ABIERTO && even_timeout && !COLGADO)
		{
			for (i=0;i<1000 && FX828.CTCSS_Detected();i++) delay_ms(1);	//El subtono tiene que estar abierto durante
			if (i>=999)													//1 segundo para ser validado.
				break;	
		}
		...
		while (!COLGADO && even_timeout)
		{
			...
			if (SQL_CERRADO) //Se cierra el SQL
			{
				FX828.EnableCTCSSDecoder (FALSE, PREDICTIVE_MODE);
				for (n=0;n<1000 && SQL_CERRADO;n++) delay_ms(1);
				if (n>=999)
				{	
					even_timeout=0;
					break;
