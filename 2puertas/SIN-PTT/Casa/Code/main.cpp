...
#define LED_PUERTA2	PTB_PTB5


extern "C" byte Terminal_SendChar(byte Chr);

unsigned int const_indicativo_puerta1[1] @0xf700;
unsigned int const_indicativo_puerta2[1] @0xf702;


const float TxCTCSSTable[]={123.0,127.3,131.8,136.5,141.3,146.2,151.4,156.7,
159.8,162.2,167.9,173.8,179.9,183.5,186.2,189.9,192.8,196.6,199.1,203.5,206.5,
...
//	FX828.Mod2Enable (FALSE);
	...
	{
		EnableTX (TRUE);
		FX828.ProgramFastCTCSS (TxCTCSSTable[APERTURA_GetVal()-1]); //Transmitimos subtono de apertura
		delay_ms (3000);
...
void comunicando (void)
{
	...
	do
	{
		if (FX828.CTCSS_Detected())
		{
			...
			for (n=0;n<1000 && FX828.CTCSS_Detected();n++) delay_ms(1); 
			if (n>999)
				break;
		}
	...
	{	
		...
		{
			if (!FX828.CTCSS_Detected())	//Perdemos el subtono de recepción
			{
				for (n=0;n<1000 && !FX828.CTCSS_Detected();n++) delay_ms(1);
				if (n>=999)
				{	
...
{
	int n;
	unsigned int indicativo_puerta1 = *const_indicativo_puerta1;
	unsigned int indicativo_puerta2 = *const_indicativo_puerta2;
	bool bSQL;
	...
	while (Terminal_SendChar ((UCHAR)HIBYTE (indicativo_puerta1))==ERR_TXFULL);
	while (Terminal_SendChar ((UCHAR)LOBYTE (indicativo_puerta1))==ERR_TXFULL);
	while (Terminal_SendChar ('\n')==ERR_TXFULL);
	while (Terminal_SendChar ((UCHAR)HIBYTE (indicativo_puerta2))==ERR_TXFULL);
	while (Terminal_SendChar ((UCHAR)LOBYTE (indicativo_puerta2))==ERR_TXFULL);
	...
	if (indicativo_puerta1==0xffff || indicativo_puerta2==0xffff )
		ErrorIndicativo();
	...
	for(;;)
	{
		...
		if (APERTURA_GetVal() && !FX828.CTCSS_Detected())
		{
			if (apertura());
				EnableTX (FALSE);
		...
		{				 
			...
			if (IndicRx.valor == indicativo_puerta2)
			{
				LED_PUERTA2 = TRUE; //Led puerta 2 encendido
				UltimaPuerta= PTB;
				GeneraTimbre();
			}
			SelCallDetected=FALSE;
