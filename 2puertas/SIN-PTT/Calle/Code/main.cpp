...
#define APERTURA_PUERTA2	PTC_PTC2
...
{	
	...
	FX828.PutToneTable (1,RxCTCSSTable[18]);	//Tono de apertura de puerta 1
	FX828.PutToneTable (2,RxCTCSSTable[19]);	//Tono de apertura de puerta 2
	...
	while (TRUE)
	{
		...
		if (bAperPuerta1==TRUE)
		{
			for (n=0;n<50 && bAperPuerta1;n++) delay_ms(1);
			if (n>=49)
			{
				APERTURA_PUERTA1=TRUE;
				delay_ms(100);
				while (bAperPuerta1)	//Mientras esté el subotono de apertura
				{
					if (!bAperPuerta1)	//Nos aseguramos de que realmente ya no hay subtono
						for (int i=0;i<500 && !bAperPuerta1;i++) delay_ms(1);
				}
				APERTURA_PUERTA1=FALSE;
			}
		}
		
		if (bAperPuerta2==TRUE)
		{
			for (n=0;n<50 && bAperPuerta2;n++) delay_ms(1);
			if (n>=49)
			{
				APERTURA_PUERTA2=TRUE;
				delay_ms(100);
				while (bAperPuerta2)	//Mientras esté el subotono de apertura
				{
					if (!bAperPuerta2)	//Nos aseguramos de que realmente ya no hay subtono
						for (int i=0;i<500 && !bAperPuerta2;i++) delay_ms(1);
				}
				APERTURA_PUERTA2=FALSE;
			}
		}
