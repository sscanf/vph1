...
byte SPI1_SendChar(byte Chr)
{
  byte i;                                                        
  for(i=0; i<8; i++) {
    Inhr6_PutVal(CLKshift);            /* Set CLK to shift value */
    Inhr5_PutVal(Chr > 127);           /* Set value on MOSI */
    InputBuffer <<= 1;
    Inhr6_PutVal(CLKsampl);            /* Set CLK to sample value */
    InputBuffer += Inhr4_GetVal()?1:0; /* Read value from MISO */
    Chr <<= 1;
  }
  if(SerFlag & CHAR_IN_RX)             /* Is char. received? */
...
byte SPI1_SetShiftClockPolarity(byte Edge)
{
  ...
  Inhr6_PutVal(CLKsampl);              /* Set CLK to idle (sample) value */
  return ERR_OK;
}
