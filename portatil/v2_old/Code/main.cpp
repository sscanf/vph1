...
unsigned int const_indicativo_puerta3[1] @0xf703;
...
void maincpp (void)
{
	...
	unsigned int indicativo_puerta3 = *const_indicativo_puerta3;
	...
	FX828.Mod1Enable (TRUE,0x01);					//Habilitamos mod1		
	FX828.Mod2Enable (TRUE,0x01);
	FX828.EnableCTCSSDecoder (FALSE, PREDICTIVE_MODE);
	FX828.EnableToneDecoder (TRUE,SELCALL_MODE);
	FX828.EnableBPF (TRUE);
	...
	{	
		...
		{				 
			...
			if (IndicRx.valor == indicativo_puerta3)
			{
				//Abrimos puerta 3
				PTB_PTB7 = TRUE;
				for (n=0;n<2000;n++)
